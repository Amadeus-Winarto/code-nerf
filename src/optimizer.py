...
class Optimizer:
    ...
    def optimize_objs(self, instance_ids, lr=1e-2, lr_half_interval=50, save_img=True):
        ...
        hpam = {
            ...
            "lr": lr,
            "lr_half_interval": lr_half_interval,
            "splits": self.splits,
        }
        ...
        self.optimized_shapecodes = torch.zeros(
            len(self.dataloader), self.mean_shape.shape[1]
        )
        self.optimized_texturecodes = torch.zeros(
            len(self.dataloader), self.mean_texture.shape[1]
        )
        # Per object
        for num_obj, d in enumerate(self.dataloader):
            ...
            shapecode = (
                self.mean_shape.to(self.device).clone().detach().requires_grad_()
            )
            texturecode = (
                self.mean_texture.to(self.device).clone().detach().requires_grad_()
            )
            ...
            while self.nopts < self.num_opts:
                ...
                for num, instance_id in enumerate(instance_ids):
                    ...
                    xyz, viewdir, z_vals = sample_from_rays(
                        rays_o,
                        viewdir,
                        self.hpams["near"],
                        self.hpams["far"],
                    ...
                    for i in range(0, xyz.shape[0], self.B):
                        sigmas, rgbs = self.model(
                            xyz[i : i + self.B].to(self.device),
                            viewdir[i : i + self.B].to(self.device),
                            shapecode,
                            texturecode,
                        )
                        rgb_rays, _ = volume_rendering(
                            sigmas, rgbs, z_vals.to(self.device)
                        )
                        # print(rgb_rays.shape, tgt_img.shape)
                        loss_l2 = torch.mean(
                            (rgb_rays - tgt_img[i : i + self.B].type_as(rgb_rays)) ** 2
                        )
                        if i == 0:
                            reg_loss = torch.norm(shapecode, dim=-1) + torch.norm(
                                texturecode, dim=-1
                            )
                            loss_reg = self.hpams["loss_reg_coef"] * torch.mean(
                                reg_loss
                            )
                ...
                self.opts.step()
                self.log_opt_psnr_time(
                    np.mean(loss_per_img),
                    time.time() - t1,
            ...
            with torch.no_grad():
                ...
                for num in range(250):
                    if num not in instance_ids:
                        ...
                        rays_o, viewdir = get_rays(
                            H.item(), W.item(), focal, poses[0, num]
                        )
                        xyz, viewdir, z_vals = sample_from_rays(
                            rays_o,
                            viewdir,
                        ...
                        for i in range(0, xyz.shape[0], self.B):
                            ...
                            loss_l2 = torch.mean(
                                (rgb_rays - tgt_img[i : i + self.B].type_as(rgb_rays))
                                ** 2
                            )
                            loss_per_img.append(loss_l2.item())
                            generated_img.append(rgb_rays)
                        self.log_eval_psnr(np.mean(loss_per_img), num, num_obj)
                        self.log_compute_ssim(
                            torch.cat(generated_img).reshape(H, W, 3),
                            tgt_img.reshape(H, W, 3),
                            num,
                            num_obj,
                        )
                        if save_img:
    ...
    def save_opts(self, num_obj):
        ...
        print("\tPSNR: ", np.mean(self.psnr_eval[num_obj]))
        print("\tSSIM: ", np.mean(self.ssim_eval[num_obj]))
    ...
    def save_img(self, generated_imgs, gt_imgs, obj_id, instance_num, opt=True):
        ...
        if opt:
            imageio.imwrite(
                os.path.join(
                    save_img_dir, "opt" + self.nviews + "_" + str(instance_num) + ".png"
                ),
                ret,
            )
        else:
            imageio.imwrite(
                os.path.join(
                    save_img_dir, str(instance_num) + "_" + self.nviews + ".png"
                ),
                ret,
            )
    ...
    def log_compute_ssim(self, generated_img, gt_img, niters, obj_idx):
        ...
        np.clip(generated_img_np, 0, 1, out=generated_img_np)
        np.clip(gt_img_np, 0, 1, out=gt_img_np)
        ssim = compute_ssim(
            generated_img_np,
            gt_img_np,
    ...
    def log_opt_psnr_time(self, loss_per_img, time_spent, niters, obj_idx):
        ...
        self.writer.add_scalar(
            "psnr_opt/" + self.nviews + "/" + self.splits, psnr, niters, obj_idx
        )
        self.writer.add_scalar(
            "time_opt/" + self.nviews + "/" + self.splits, time_spent, niters, obj_idx
        )

    def log_regloss(self, loss_reg, niters, obj_idx):
        self.writer.add_scalar(
            "reg/" + self.nviews + "/" + self.splits, loss_reg, niters, obj_idx
        )
    ...
    def set_optimizers(self, shapecode, texturecode):
        ...
        self.opts = torch.optim.AdamW(
            [{"params": shapecode, "lr": lr}, {"params": texturecode, "lr": lr}]
        )
    ...
    def load_model_codes(self, saved_dir):
        ...
        self.mean_shape = torch.mean(
            saved_data["shape_code_params"]["weight"], dim=0
        ).reshape(1, -1)
        self.mean_texture = torch.mean(
            saved_data["texture_code_params"]["weight"], dim=0
        ).reshape(1, -1)
